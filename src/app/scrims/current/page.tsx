...
export default async function CurrentScrimsPage() {
  const workspace = await getScrimWorkspace();
  const currentCount = workspace.upcomingScrims.length;
  const currentUnreadChatCount = workspace.upcomingScrims.reduce(
    (total, scrim) => total + scrim.unreadChatCount,
    0,
  );
  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-12 sm:px-6 lg:px-8">
      <ScrimNav
        ...
        currentUnreadChatCount={currentUnreadChatCount}
        ...
        <>
          <section className="flex flex-col gap-3 border-b border-line/60 pb-5">
            ...
            <p className="max-w-3xl text-sm leading-6 text-muted">
              Accepted scrims live here after they leave the incoming request
              queue.
            </p>
          </section>
          ...
          <section className="grid gap-6 xl:grid-cols-[1fr_0.55fr]">
            <div className="surface rounded-lg p-6">
              ...
              <h2 className="mt-2 font-display text-3xl font-bold text-white">
                ...
                {currentUnreadChatCount > 0 ? (
                  <span className="ml-3 inline-flex align-middle rounded-full bg-success px-2.5 py-1 text-sm font-semibold text-slate-950">
            ...
            <aside className="surface rounded-lg p-6">
              ...
              <p className="mt-4 text-sm leading-6 text-muted">
                Incoming scrims now only shows teams actively requesting a new
                scrim from your team.
              </p>
              <Link
