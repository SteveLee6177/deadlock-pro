...
export default async function FindScrimsPage() {
  const [teams, blocks, workspace] = await Promise.all([
    getCurrentScrimTeams(),
    getOpenAvailabilityBlocks(),
    getScrimWorkspace(),
  ]);
  ...
  const manageableTeamIds = teams
    .filter((team) => team.canManageScrims)
    .map((team) => team.id);
  const visibleBlocks = await removeDeclinedMatchupBlocks(
    blocks.filter((block) => {
      if (!ownTeamIds.has(block.teamId)) {
        return true;
      }

      return manageableTeamIds.some((teamId) => teamId !== block.teamId);
    }),
      ...
      <section className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-3 font-display text-4xl font-bold tracking-tight text-white">
            {visibleBlocks.length} open scrim block
            {visibleBlocks.length === 1 ? "" : "s"}
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
            Open blocks are hidden only when none of your manageable teams can
            request them.
          </p>
        </div>
        {teams.length === 0 ? (
